...
uniform float sharpness_h : hint_range(0.0, 1.0, 0.05) = 0.0; // 0.5
uniform float sharpness_v : hint_range(0.0, 1.0, 0.05) = 0.0; // 1.0
uniform float mask_strength : hint_range(0.0, 1.0, 0.01) = 0.05; // 0.3
uniform float mask_dot_width : hint_range(1.0, 100.0, 0.1) = 2.0; // 1.0
uniform float mask_dot_height : hint_range(1.0, 100.0, 0.1) = 2.0; // 1.0
uniform float mask_stagger : hint_range(0.0, 100.0, 1.0) = 9.0; // 0.0
uniform float mask_size : hint_range(0.1, 100.0, 0.1) = 0.5; // 1.0
uniform float scanline_strength : hint_range(0.0, 1.0, 0.05) = 1.0; // 1.0
uniform float scanline_beam_width_min : hint_range(0.5, 5.0, 0.5) = 1.5; // 1.5
uniform float scanline_beam_width_max : hint_range(0.5, 5.0, 0.5) = 1.5; // 1.5
uniform float scanline_bright_min : hint_range(0.0, 1.0, 0.05) = 0.35; // 0.35
uniform float scanline_bright_max : hint_range(0.0, 1.0, 0.05) = 0.5; // 0.65
uniform float scanline_cutoff : hint_range(1.0, 1000.0, 1.0) = 400.0; // 400.0
uniform float gamma_input : hint_range(0.1, 5.0, 0.1) = 0.1; // 2.0
uniform float gamma_output : hint_range(0.1, 5.0, 0.1) = 0.1; // 1.8
uniform float bright_boost : hint_range(1.0, 2.0, 0.01) = 1.5; // 1.2
uniform float dilation : hint_range(0.0, 1.0, 1.0) = 1.0; // 1.0
